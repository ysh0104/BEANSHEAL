...
/** 재고수불부 검색 조건 (기준일자·재고수량 없음) */
export async function isLedgerSearchForm(page: Page): Promise<boolean> {
  if (await isStockSearchForm(page) || (await isStockResultsReady(page))) return false;
...
/** 재고수불부 결과 + Excel (재고 isStockResultsReady 와 동일 패턴) */
export async function isLedgerResultsReady(page: Page): Promise<boolean> {
  if (await isLedgerSearchForm(page) || (await isStockResultsReady(page))) return false;

  for (const frame of page.frames()) {
    try {
      const partner = frame.locator("text=거래처명").first();
      const inQty = frame.getByText(/입고수량/).first();
      const hasHeader =
        ((await partner.count()) > 0 && (await partner.isVisible())) ||
        ((await inQty.count()) > 0 && (await inQty.isVisible()));
...
export async function clickLedgerExcelDownload(page: Page, saveAs: string): Promise<void> {
  ...
  if (!(await isLedgerResultsReady(page))) {
    const visible = await findVisibleExcelButton(page);
    if (!visible) throw new Error("LEDGER_NOT_READY");
  }
